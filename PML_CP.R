...
# no            - impute any fields that may be missing small amounts of data
# DONE         - split the data into a 'training' and 'testing' set
# DONE  3) Figure out what data is not importanct
# no            - exploratory plots
# DONE          - preliminary random forests model to screen variables
# no            - perform correlations to see if we can use PCA on some variables
# DONE  4) Create another model to finalize the first one--> but only use the variables that pass section (3)
# DONE          -  further split the 'training' data into two:  'training' and 'training.test' the latter will be for cross validation
# DONE          -  complete further testing and finalize the models
# no            -  calculate the 'in sample' error
# DONE  5) Test the new model on the 'testing' dataset
...
library(caret); library(rattle); library(ggplot2); set.seed(12142014)    
...
inTrain    <-createDataPartition(data$classe, p=.60, list = FALSE)               # create inTrain string for data partitions   (60:40 split)
training   <-data[inTrain,]                                                      # split data into training and testing
###################################################################################################################################################
#
# Section 3) Figure out what data is not important
library(doMC)                                                                   # added to enable all 4 cores
registerDoMC(cores = 4)
# All subsequent models are then run in parallel

model<-train(classe ~., data=training, method='rf', importance=TRUE)            # 1st random forest--> 12K obs with 53 variables....started at 3:47 to 4:19
vi<-varImp(model)
plot(vi)
vi_vals<-as.data.frame(vi[1])
